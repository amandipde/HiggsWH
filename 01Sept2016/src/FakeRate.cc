...
{
  new TH1F("npu", "npu distrubution//MC only", 50, 0, 50);
  new TH1F("truenint", "no of true int distrubution//MC only", 50, 0, 50);
  new TH1F("npv", "npv distrubution", 50, 0, 50);
  new TH1F("nPVCorrected", "npv distrubution", 50, 0, 50);
  new TH1F("npv_PU", "npv distrubution with PU correction applied//MC only", 50, 0, 50);
  new TH1F("puwt", "PU weight factor", 20, 0, 20);
  ...
  new TH1F("faketaupt_deno_WJetsAll", "Denominator of the JetToTau Fake Function", 140, 0, 140);
  new TH1F("faketaupt_deno_WJetsC", "Denominator of the JetToTau Fake Function", 140, 0, 140);
  new TH1F("faketaupt_deno_WJetsI", "Denominator of the JetToTau Fake Function", 140, 0, 140);
  new TH1F("faketaupt_deno_WJetsF", "Denominator of the JetToTau Fake Function", 140, 0, 140);
  ...
  new TH1F("faketaupt_nume_WJetsAll", "Tight WP Numerator of the JetToTau Fake Function", 140, 0, 140);
  new TH1F("faketaupt_nume_WJetsC", "Tight WP Numerator of the JetToTau Fake Function", 140, 0, 140);
  new TH1F("faketaupt_nume_WJetsI", "Medium WP Numerator of the JetToTau Fake Function", 140, 0, 140);
  new TH1F("faketaupt_nume_WJetsF", "Medium WP Numerator of the JetToTau Fake Function", 140, 0, 140);
...
void FakeRate::selectTau()                
{
  for (auto it = tauColl()->begin(); it != tauColl()->end(); ++it) {
    ...
    if ((fabs(tau.eta) >= AnaUtil::cutValue(_tau1CutMap, "eta")) ||
        ...
        (tau.againstElectronLooseMVA6 <= 0.5)
       ) continue; 
...
void FakeRate::JetToFakeDYJets()
{
  ...
  if (muo1.pt <= 22 || muo2.pt <= 15) return;               
  ...
  sum = M1 + M2;
  if (AnaUtil::deltaR(M1, M2) < 0.1) return;
  ...
  for (unsigned int indx = 0; indx < selectTauList.size(); ++indx) {
    ...
    if (fabs(fake.eta) < 0.8) { 
      ...
      if (fake.byTightCombinedIsolationDeltaBetaCorr3Hits > 0.5)
    ...
    if (fabs(fake.eta) <= 2.3) {
      ...
      if (closureCountDY == 1.0){
        if (fake.byTightCombinedIsolationDeltaBetaCorr3Hits > 0.5) AnaUtil::fillHist1D("faketaupt_Isolated_DYAll", fake.pt, puevWt_);
...
void FakeRate::JetToTauFakeWJets()
{
  ...
  if (mass1 <= 50) return;//changed to 40->50 to improve the purity of control region                                                          
  ...
  */
  ...
  if (ta1.charge == ta2.charge && muo1.charge == ta1.charge) { 
    for (unsigned int indx = 0; indx < 2; ++indx) {
      ...
      if (fabs(fake.eta) < 0.8) {
        AnaUtil::fillHist1D("faketaupt_deno_WJetsC", fake.pt, puevWt_);
        if (fake.byTightCombinedIsolationDeltaBetaCorr3Hits > 0.5)
          AnaUtil::fillHist1D("faketaupt_nume_WJetsC", fake.pt, puevWt_);
      }

      if (fabs(fake.eta) >= 0.8 && fabs(fake.eta) < 1.6) {
        AnaUtil::fillHist1D("faketaupt_deno_WJetsI", fake.pt, puevWt_);
        if (fake.byTightCombinedIsolationDeltaBetaCorr3Hits > 0.5)
          AnaUtil::fillHist1D("faketaupt_nume_WJetsI", fake.pt, puevWt_);
      }

      if (fabs(fake.eta) >= 1.6 && fabs(fake.eta) < 2.3) {
        AnaUtil::fillHist1D("faketaupt_deno_WJetsF", fake.pt, puevWt_);
        if (fake.byTightCombinedIsolationDeltaBetaCorr3Hits > 0.5)
          AnaUtil::fillHist1D("faketaupt_nume_WJetsF", fake.pt, puevWt_);
      }
      if (fabs(fake.eta) < 2.3) {
        AnaUtil::fillHist1D("faketaupt_deno_WJetsAll", fake.pt, puevWt_);
        if (fake.byTightCombinedIsolationDeltaBetaCorr3Hits > 0.5)
          AnaUtil::fillHist1D("faketaupt_nume_WJetsAll", fake.pt, puevWt_);
        if (fake.byTightCombinedIsolationDeltaBetaCorr3Hits > 0.5) AnaUtil::fillHist1D("faketaupt_Isolated_WJetsAll", fake.pt, puevWt_);
...
*/
  ...
  for (unsigned int indx = 0; indx < selectTauList.size(); ++indx) {
    ...
    double vtxdz = abs(tau.zvertex - vz);
    ...
    //if (decision1 &&  tau.byMediumCombinedIsolationDeltaBetaCorr3Hits > 0.5) tau1List.push_back(tau);
  }
  ...
  for (unsigned int indx = 0; indx < selectTauList.size(); ++indx) {
    ...
    //if (decision2 && tau.byTightCombinedIsolationDeltaBetaCorr3Hits > 0.5) tau2List.push_back(tau);
  }
  ...
  for (unsigned int indx = 0; indx < tauOSList.size(); ++indx) {
    ...
    if (tau.byMediumCombinedIsolationDeltaBetaCorr3Hits > 0.5) tau1List.push_back(tau);
    else antitau1List.push_back(tau);
  }
  for (unsigned int indx = 0; indx < tauSSList.size(); ++indx) {
    const Tau& tau = tauSSList[indx];

    if (tau.byTightCombinedIsolationDeltaBetaCorr3Hits > 0.5) tau2List.push_back(tau);
  ...
  if (tau1List.size() > 0 && tau2List.size() > 0) {
    for (unsigned int indx = 0; indx < tau1List.size(); ++indx) {
      ...
      for (unsigned int jndx = 0; jndx < tau2List.size(); ++jndx) {
        ...
        if (AnaUtil::deltaR(T1, T2) >= 0.1 && (tau1.pt > AnaUtil::cutValue(_tau1CutMap, "pt") || tau2.pt > AnaUtil::cutValue(_tau1CutMap, "pt"))) {
          tau2_indx = jndx;
  ...
  /*
  //force both of the anti legs to have more that tau1 pt
  bool foundantiantiTauPair = false;
  if (!foundtaupair && tau1List.size() > 0 && tau2List.size() > 0 && antitau1List.size() > 0 && antitau2List.size() > 0) {
    for (unsigned int indx = 0; indx < antitau1List.size(); ++indx) {
      const Tau& tau1 = antitau1List.at(indx);
      T1.SetPtEtaPhiE(tau1.pt, tau1.eta, tau1.phi, tau1.energy);
      for (unsigned int jndx = 0; jndx < antitau2List.size(); ++jndx) {
        const Tau& tau2 = antitau2List.at(jndx);
        T2.SetPtEtaPhiE(tau2.pt, tau2.eta, tau2.phi, tau2.energy);
        if (AnaUtil::deltaR(T1, T2) >= 0.1 && (tau1.pt > AnaUtil::cutValue(_tau1CutMap, "pt") && tau2.pt > AnaUtil::cutValue(_tau1CutMap, "pt"))) {
          tau2_indx = jndx;
          taubzvtx = tau2.zvertex;
          foundantiantiTauPair = true;
          break;
        }
      }
      if (foundantiantiTauPair) {
        tau1_indx = indx;
        tauazvtx = tau1.zvertex;
        break;
      }
  ...
  bool antiTau1 = false;
  if (!foundtaupair && tau1List.size() > 0 && tau2List.size() > 0 && antitau1List.size() > 0) {
    for (unsigned int indx = 0; indx < tau2List.size(); ++indx) {
      const Tau& tau1 = tau2List.at(indx);
      T1.SetPtEtaPhiE(tau1.pt, tau1.eta, tau1.phi, tau1.energy);
      for (unsigned int jndx = 0; jndx < antitau1List.size(); ++jndx) {
        const Tau& tau2 = antitau1List.at(jndx);
        T2.SetPtEtaPhiE(tau2.pt, tau2.eta, tau2.phi, tau2.energy);
        if (AnaUtil::deltaR(T1, T2) >= 0.1 && (tau1.pt > AnaUtil::cutValue(_tau1CutMap, "pt") || tau2.pt > AnaUtil::cutValue(_tau1CutMap, "pt"))) {
          tau1_indx = jndx;
          tauazvtx = tau2.zvertex;
          antiTau1 = true;
          break;
        }
      }
  ...
  bool antiTau2 = false;
  if (!foundtaupair && !antiTau1 && tau1List.size() > 0 && tau2List.size() > 0 && antitau2List.size() > 0) {
    for (unsigned int indx = 0; indx < tau1List.size(); ++indx) {
      const Tau& tau1 = tau1List.at(indx);
      T1.SetPtEtaPhiE(tau1.pt, tau1.eta, tau1.phi, tau1.energy);
      for (unsigned int jndx = 0; jndx < antitau2List.size(); ++jndx) {
        const Tau& tau2 = antitau2List.at(jndx);
        T2.SetPtEtaPhiE(tau2.pt, tau2.eta, tau2.phi, tau2.energy);
  ...
  */
  ...
  //
  //---------------------------------------------
  if (vetoMuon(tauazvtx, 15, 0.14) != 1) return;
  ...
  //
  //---------------------------------------------
  if (vetoElectron(tauazvtx, 10, 0.14) != 0) return;
  ...
  if (mass1 <= 40) return; //changed 30 ->40, temporarily
...
void FakeRate::MuMuTauApplyFR()
{
  ...
  bool tauIso = (taua.byMediumCombinedIsolationDeltaBetaCorr3Hits > 0.5);
